...
## 대학생을 위한 메이커 중심 AI 리터러시 프로그램

> **"먼저 체험하고, 분해하고, 직접 만들어보자!"**  
> V0, Cursor로 가상환경을 구축하며 배우는 실전 AI 개발 교육
...
1. [프로그램 개요](#프로그램-개요)
2. [메이커 교육 철학](#메이커-교육-철학)
3. [바이브코딩 학습 방식](#바이브코딩-학습-방식)
4. [3대 소주제 - 대학생 맞춤형](#3대-소주제)
5. [학습자 페르소나](#학습자-페르소나)
6. [AI-LLM 3단계 시스템](#ai-llm-3단계-시스템)
7. [메이커 프로세스 로드맵](#메이커-프로세스-로드맵)
8. [AI 도구 활용 가이드](#ai-도구-활용-가이드)
9. [교수자 코칭 가이드](#교수자-코칭-가이드)
10. [학습 시나리오](#학습-시나리오)
...
**"완성작을 먼저 경험하고, 역으로 분해하며, 스스로 만들어내는 힘을 키운다"**

대학생들이 흥미로워하는 주제(주식 투자, 연애, 창업)를 가상환경으로 구현하면서 **AI 기획자**이자 **실행자**로 성장하는 메이커 중심 프로그램

### 🎓 교육 대상 및 목표

| 구분 | 내용 |
|------|------|
| **대상** | 대학생 (전공 무관, AI 도구에 관심 있는 학습자) |
| **기간** | 각 소주제당 6-8주 (총 18-24주) |
| **방식** | 체험 → 분해 → 재구성 → 창조 (메이커 사이클) |
| **결과물** | 실제 작동하는 웹 시뮬레이터 3개 + 포트폴리오 |
| **도구** | V0, Cursor, ChatGPT, Claude 등 생성형 AI |
...
| 평가 요소 | 전통적 교육 | 메이커 교육 |
|----------|-----------|-----------|
| **평가 대상** | 시험 점수 | 작동하는 프로젝트 |
| **평가 시점** | 학기말 | 매주 진행 상황 |
| **평가 기준** | 정답 여부 | 과정 + 결과물 + 창의성 |
| **평가자** | 교수만 | 교수 + 동료 + 자가평가 |
...
"코딩을 몰라도 웹앱을 만들 수 있다는 게 신기했어요" - 경영학과 민지

"AI 도구 쓰는 법을 알게 돼서 취업에 도움될 것 같아요" - 컴공과 준호

"실제로 작동하는 걸 만들어서 자신감이 생겼어요" - 디자인과 수민

"이제 아이디어가 있으면 직접 만들 수 있을 것 같아요" - 사회학과 지훈
```
...
**1. 코딩이 아니라 사고방식을 가르친다**
- 전통적 코딩 교육: 문법 → 알고리즘 → 프로젝트
- 메이커 교육: 결과 체험 → 역설계 → AI로 구현
...
**Q1. 코딩을 전혀 모르는 학생도 가능한가요?**  
A. 네! 오히려 코딩 경험이 없는 학생이 선입견 없이 더 잘 배웁니다. AI 도구가 코딩을 대신하므로 기획 능력과 사고력이 더 중요합니다.

**Q2. 8주가 너무 짧지 않나요?**  
A. 완성도 높은 1개 프로젝트를 만들기에 적당합니다. 3개 프로젝트를 원하면 24주 과정으로 확장하세요.

**Q3. AI 도구 비용이 부담스럽습니다**  
A. 대부분 무료 크레딧으로 가능합니다. OpenAI $5, V0 무료, Cursor 학생 할인이 있습니다.

**Q4. 전공별로 다른 주제가 필요한가요?**  
A. 3개 주제 모두 전공 무관하게 적용 가능합니다. 창업 시뮬레이터는 경영/창업 전공에게, 연애 시뮬레이터는 심리/사회학과에게 특히 인기입니다.

**Q5. 평가는 어떻게 하나요?**  
A. 작동하는 프로젝트 + GitHub 문서 + 발표로 평가합니다. 과정을 중시하므로 완벽하지 않아도 괜찮습니다.
...
```markdown
📧 문의: vivecoding.ai@example.com

제목에 [가상환경 메이커 교육 문의]를 명시해주세요.
...
**Version:** 2.0.0 (메이커 교육 버전)  
**Author:** 바이브코딩 AI 교육팀  
**Target:** 대학생 / 코딩 입문자 / AI 리터러시 교육
